...
of $f_\phi$ to satisfy (\ref{allfunction}). This simple, elegant argument has two
...
in a combinatorial way how to transform proofs using (\ref{allfunction}) into proofs
using  (\ref{allexists}).  Here and below we restrict attention to
...
Such a proof transformation exists and has been found by Maehara~\cite{Maehara}. 
A modern account can be found in, for example, Gallier~\cite[Ch.~7]{Gallier}.
It may come as a surprise that these proof transformations are quite complicated.
