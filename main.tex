...
This case is essentially simpler, since now (\ref{allskol}) is the only axiom
...
Let $\rho = \rho_{f({t}),u}$ and let $T$ consist of axioms not
mentioning $f$ plus the $f$-axiom $\vdash_{y} P(y,f(y))$.
Assume all free variables of $\vec{t}$ are in $\vec{x}$. If
$\Gamma\vdash_{\vec{x}}^T A$ by a derivation not using the instance $P({t},f({t}))$
of the $f$-axiom, then $\Gamma\rho\vdash_{\vec{x},u}^T A\rho$.
%by a derivation of the same size.
\end{lemma}

\begin{proof}
The proof follows the proof of Lemma~\ref{subst2}. The only point requiring extra 
attention is the (possible) use of the axiom $\vdash_{{y}} P({y},f({y}))$.
...
$f(s_i,t_i)$ is not in $\Gamma,A,s_j,t_j$ for $1\leq j < i \leq n$,
