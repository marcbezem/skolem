...
The examples in this section are examples where the
skolemized sentence is not true in the generic model,
so that their conservativity does not follow from elementary semantic observations.
...
We will elaborate the example \ref{all2exist2} from the introduction, 
which will turn out to fall under point 2 above, but not under point 3.
Consider the language with $=$ and a binary relation $P$. Let the theory $T$ consist of
$\vdash_x \exists y~P(x,y)$. Add a function symbol $f$ with a congruence axiom
...
This can be proved in different ways: Mints~\cite{Mints} gives a proof-theoretic argument.
There exists a simple Kripke countermodel. Here we show that $(**)$ is
not valid in the generic model of $T$. Hence $(**)$ is a negative answer to the question
of Gavin Wraith. 
