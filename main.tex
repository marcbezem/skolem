...
Let $T_f$ be $T$ extended by the congruence axiom for a new function symbol $f$,
$\vec{x}=\vec{y} \vdash_{\vec{x},\vec{y}} f(\vec{x}) = f(\vec{y})$. 
Then we have $\ldf{\Gamma}\vdash_{\vec{x}}^T \ldf{A}$
whenever $\Gamma\vdash_{\vec{x}}^{T_f} A$.
...
We let $T_f$ be the theory like $T$ but where we
add the symbol function $f$ to the signature.
Let $Tf^+$ be the extension of $T_f$ with the rule
$$
\vdash_{y}~P(y,f(y))\leqno{(*)}
$$

We prove that $T_{f{+}}$ is a conservative extension of $T_f$,
which is a conservative extension of $T$ by Lemma~\ref{subst3}.
The main idea is that if we use $f$ and the rule
$(*)$ with $P(t,f(t))$ we should be able to replace $f(t)$ by a fresh variable 
$P(t,u)$ and then we can use the fact that $\exists u~P(t,u)$ is provable.
...
$\Gamma,P(t,u)\vdash_{\vec{x},u}^{T_0} A$. Since we also have
...
Total preorders with a ternary predicate for maxima can be axiomatized by:
\begin{align*}
...
do this for the example, the method is completely general.
...
for the lone variable $u$ and get $\Gamma,M(x,y,x) \vdash_{\vec{x},u}^{T_1} A$
and $\Gamma,M(x,y,y) \vdash_{\vec{x},u}^{T_1} A$, respectively.
Hence we also have $\Gamma \vdash_{\vec{x},u}^{T_1} A$
...
\end{lemma}
\begin{proof}
By induction on derivation. Applications of the congruence axiom for $f$
can be left out.
\end{proof}
...
the following axioms, where  $I(x_1,x_2,y_1,y_2)$ is a new predicate
expressing $x_1=x_2 \to y_1=y_2$:
\begin{align*}
...
\end{align*}
Here $V$ consists of the variables $x_1,x_2,y_1,y_2$. In theories with
implicational equality we denote the congruence axiom for a unary
function $f$ by $\vdash_{x,x'} I(x,x',f(x),f(x'))$.
\end{definition}
Implicational equality is conservative over equality, which can be seen be a
cut-elimination argument. Alternatively, one can substitute $x_1=x_2 \to y_1=y_2$
for $I(x_1,x_2,y_1,y_2)$ and observe that the axioms above become
classical tautologies. The cut-elimination argument is constructive, whereas
the alternative argument uses the conservativity of classical logic over
coherent logic (Barr's Theorem), which is not constructive.

\begin{lemma}\label{lem:main}
Let $T$ be a coherent theory with implicational equality consisting of
axioms not mentioning $f$ plus the $f$-axioms $\vdash_{{y}} P({y},f({y}))$
and $\vdash_{x,x'} I(x,x',f(x),f(x'))$. Let $T_0$ be the coherent theory obtained from 
$T$ by leaving out the $f$-axioms and adding $\vdash_{\vec{y}} \exists u P({y},u)$.
Then we have:
\begin{enumerate}
\item If $\Gamma,P(t,f(t)), I(t,u_1,f(t),f(u_1)),\ldots,I(t,u_n,f(t),f(u_n))\vdash_{\vec{x}}^{T_0} A$
and $f(t)$ does not occur in $\Gamma,A$, $u_1,\ldots,u_n$, then  
$\Gamma,\Sigma\vdash_{\vec{x}}^{T_0} A$, where $\Sigma$ consists of 
all $P(u_i,f(u_i))$ and $I(u_i,u_j,f(u_i),f(u_j))$;
\item If $\Gamma\vdash_{\vec{x}}^T A$ and $\Sigma$ contains all $P(t,f(t))$ and
$I(t,u,f(t),f(u))$ such that $f(t)$ and $f(u)$ occur in $\Gamma,A$,
then $\Gamma,\Sigma\vdash_{\vec{x}}^{T_0} A$.
\end{enumerate}
\end{lemma}
...
\Gamma,P(t,y),  I(t,u_1,y,f(u_1)),\ldots,I(t,u_n,y,f(u_n))\vdash_{\vec{x},y}^{T_0} A
\end{equation}
Note that $t=u_i,y=f(u_i),\Gamma,\Sigma\vdash_{\vec{x},y}^{T_0} A$
follows from (\ref{eq:ft=y}) by equality reasoning, for any $i$. 
By substituting $f(u_i)$ for the
lone variable $y$ we get $(\ast)~t=u_i,\Gamma,\Sigma\vdash_{\vec{x}}^{T_0} A$
by Lemma~\ref{subst1} and reflexivity.
We now build a proof of $\Gamma,\Sigma\vdash_{\vec{x}}^{T_0} A$ from first 
$\Gamma,\Sigma,P(t,y)\vdash_{\vec{x},y}^{T_0} A$ and then repeated
...
2. By induction on the derivation of $\Gamma\vdash_{\vec{x}}^T A$.
There are three cases to distinguish, depending on the last step in the derivation.

Case $P(t,f(t))$. Then the IH gives
\begin{equation}\label{ih:Ptft}
\Gamma,\Sigma,P(t,f(t)), I(t,u_1,f(t),f(u_1)),\ldots,I(t,u_n,f(t),f(u_n))\vdash_{\vec{x}}^{T_0} A
\end{equation}
where $\Sigma$ consists of all $P(u_i,f(u_i))$ and $I(u_i,u_j,f(u_i),f(u_j))$
with $f(u_i),f(u_j)$ in $\Gamma,A$. If $f(t)$ occurs in $\Gamma,A$ we are done.
Otherwise we apply part 1 and we are also done.

Case $I(s,t,f(s),f(t))$. Then the IH gives
\begin{equation}\label{ih:Istfsft}
\Gamma,I(s,t,f(s),f(t)),\Sigma'\vdash_{\vec{x}}^{T_0} A
\end{equation}
where $\Sigma'$ consists of all $P(u_i,f(u_i))$ and $I(u_i,u_j,f(u_i),f(u_j))$
with $f(u_i),f(u_j)$ in $\Gamma,A,f(s),f(t)$. 
Let $\Sigma$ consists of all $P(u_i,f(u_i))$ and $I(u_i,u_j,f(u_i),f(u_j))$
with $f(u_i),f(u_j)$ in $\Gamma,A$. We have to prove 
...
\begin{lemma}\label{lem:up_f_ront}
Let $T$ with equality consist of axioms not mentioning $f$ 
%including $\vdash_{\vec{y}} \exists u P({y},u)$,  
plus the $f$-axioms $\vdash_{{y}} P({y},f({y}))$
and $x=x'\vdash_{x,x'} f(x) = f(x')$.
Assume $\Gamma\vdash_{\vec{x}}^T A$. 
Let $\Sigma$ consist of atoms $P(s,f(s))$ and $f(s) = f(s')$
with all free variables in $\vec{x}$.
Then $\Gamma,\Sigma\vdash_{\vec{x}}^T A$
by a derivation not using the instances of the $f$-axioms 
concluding atoms in $\Sigma$.
\end{lemma}

\begin{proof}
Similarly to Lemma~\ref{lem:upfront},
applications of $f$-axioms concluding atoms in $\Sigma$ can be left out.
\end{proof}

\medskip

The following definition is inspired by the failure of Lemma~\ref{simpl}
for $a=b,P(a,f(a)) \vdash P(a,f(b))$ (NB $f(a)$ not in $a=b,P(a,f(b))$), 
as well as by the above example based on (\ref{all2exist2}). EXPERIMENTAL!

\begin{definition}\label{def:ft_safety}
Let $T_0$ be a theory with equality such that the only axiom mentioning $f$
is the congruence axiom $x=x'\vdash_{x,x'} f(x) = f(x')$.
Assume all free variables of $\vec{t}$ are in $\vec{x}$. 
A derivation of  $\Gamma \vdash_{\vec{x}}^{T_0} A$ is called $f(t)$-\emph{safe}
if for every instance of $x=x'\vdash_{x,x'} f(x) = f(x')$ with $x,x'=s,s'$ 
that is used in the derivation, we have $\Gamma \vdash_{\vec{x}}^{T_0} t=s'$
if and only if $\Gamma \vdash_{\vec{x}}^{T_0} t=s$.
$\Gamma,A$  is called $f(t)$-\emph{safe} if $\Gamma,A$ does not mention 
any term $f(s)$ such that $\Gamma \vdash_{\vec{x}}^{T_0} t=s$.
\end{definition}

\begin{lemma}\label{lem:subst_all}
Let $T_0$ be as in Definition~\ref{def:ft_safety}.
Assume all free variables of ${t}$ are in $\vec{x}$.
Let $\rho$ be the substitution replacing all maximal terms
$f(s)$ such that $\Gamma\vdash_{\vec{x}}^{T_0} t=s$ by a new variable $u$.
If $\Gamma\vdash_{\vec{x}}^{T_0} A$ is $f(t)$-safe,
then $\Gamma\rho \vdash_{\vec{x}}^{T_0} A\rho$. DECIDABLE???
\end{lemma}

\begin{proof}
By induction on derivation.
CHECK The only interesting case is the congruence axiom for $f$, inferring
$f(s)=f(s')$ from $s=s'$. By $f(t)$-safety we either get a conclusion $u=u$,
which can be obtained by reflexivity, or $f(s\rho)=f(s)\rho=f(s')\rho=f(s'\rho)$,
which follows from $s\rho=s'\rho$.
\end{proof}

\begin{lemma}\label{lem:T0}
Let $T_0$ be as in Definition~\ref{def:ft_safety},
including the axiom $\vdash_{{y}} \exists u P({y},u)$. 
Assume all free variables of ${t}$ are in $\vec{x}$.
Let $\Sigma$ consist of atoms $P(s,f(s))$ and $f(s) = f(s')$
such that $\Gamma \vdash_{\vec{x}}^{T_0} t=s~(=s')$.
If $\Gamma,\Sigma\vdash_{\vec{x}}^{T_0} A$ is $f(t)$-safe,
 then $\Gamma\vdash_{\vec{x}}^{T_0} A$.
\end{lemma}
...
\begin{lemma}\label{lem:elim_ft}
Let $T$ with equality consist of axioms not mentioning $f$, 
%including the axiom $\vdash_{{y}} \exists u P({y},u)$,  
plus the $f$-axioms $\vdash_{{y}} P({y},f({y}))$
and $x=x'\vdash_{x,x'} f(x) = f(x')$.
Assume all free variables of ${t}$ are in $\vec{x}$.
Let $\rho$ be the substitution replacing all terms
$f(s)$ such that $\Gamma\vdash_{\vec{x}}^T t=s$ by a new variable $u$.
Consider a derivation of $\Gamma\vdash_{\vec{x}}^T A$
that is $f(t)$-safe. Then there is a derivation
of $\Gamma\rho\vdash_{\vec{x},u}^T A\rho$ that is
not using any instances of the $f$-axioms with a term $s$
such that $\Gamma \vdash_{\vec{x}}^{T} t=s$.
\end{lemma}

\begin{proof}
Similar to Lemma~\ref{subst3}.
\end{proof}
...
and $x=x'\vdash_{x,x'} f(x) = f(x')$.
Assume all free variables of ${t}$ are in $\vec{x}$, 
$\Gamma,A$ is $f(t)$-safe, and $\Gamma\vdash_{\vec{x}}^T A$.
Then there is a derivation
of $\Gamma\vdash_{\vec{x}}^T A$ that is
not using any instances of the $f$-axioms with a term $s$
such that $\Gamma \vdash_{\vec{x}}^{T} t=s$.
\end{lemma}

\begin{proof}
Let $\Sigma$ be the set of all atoms $P(s,f(s))$ and $f(s) = f(s')$
inferred in the derivation of  $\Gamma\vdash_{\vec{x}}^T A$ by 
an $f$-axiom, and such that $\Gamma \vdash_{\vec{x}}^{T} t=s$. 
For $f(s) = f(s')$ it follows that $\Gamma \vdash_{\vec{x}}^{T} t=s'$ by $f(t)$-safety.
By Lemma~\ref{lem:up_f_ront} we get that $\Gamma,\Sigma\vdash_{\vec{x}}^T A$
by a derivation not using the instances of the $f$-axioms concluding atoms in $\Sigma$.
By Lemma~\ref{lem:elim_ft} it follows that $\Gamma,\Sigma\rho\vdash_{\vec{x},u}^T A$
by a derivation not using the instances of the $f$-axioms concluding atoms in $\Sigma$.
The atoms in $\Sigma\rho$ are either of the form $P(s,u)$, or $u=u$. The latter can be left out.
Of the former we know that $\Gamma \vdash_{\vec{x}}^{T} t=s$.
Therefore we can infer the first atom $P(s,u)$ with the axiom
$\vdash_{{y}} P({y},f({y}))$, after which all subsequent atoms follow by congruence.
This gives a derivation of  $\Gamma\vdash_{\vec{x}}^T A$ as desired.
\end{proof}


\medskip

 It is remarkable that this proof transformation is still simple (but there can
be a combinatorial explosion), while
...
get a theory $T_{f{+}}$ which is conservative over $T$ for {\em coherent} formulae. 

 This theory however is {\em not} conservative for arbitrary formulae since we can prove in
$T_{f{+}}$ the non coherent formula
$$
