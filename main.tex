...
(\ref{allfunction}) not containing $f_\phi$ already follow from (\ref{allexists}). 
...
Despite its elegance, the above argument is unsatisfactory from the
proof theoretic point of view. One would like to understand
how to transform proofs using (\ref{allfunction}) into proofs
using  (\ref{allexists}) in a combinatorial way, bypassing the use 
of the Axiom of Choice. (Here and below we restrict attention to
proofs with conclusion not containing $f_\phi$.)
Such a proof transformation must exist and has been found by Maehara~\cite{Maehara}. 
A modern account can be found in, for example, Gallier~\cite[Ch.~7]{Gallier}.
It may come as a surprise that these proof transformations are rather 
