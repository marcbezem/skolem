...
Here $f$ must have positive arity (the case of a 
...
an application of $\vdash_{\vec{x}} P(\vec{t},f(\vec{t}))$
...
Assume all free variables of $\vec{t}$ are in $\vec{x}$.
If $\Gamma,A$ does not mention $f(\vec{t})$ 
and $\Gamma\vdash_{\vec{x}}^T A$, 
...
Now apply $(\Gamma,A)\rho=\Gamma,A$ and
use the axiom $\vdash_{\vec{y}} \exists u P(\vec{y},u)$ to get
