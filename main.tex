...
in theories with implicational equality we denote the congruence axiom for 
...
$\vdash_{x} \exists u P(x,u)$ instead of the $f$-axioms mentioning 
in (\ref{faxioms}) above? If we start by $P(a,u), P(b,v)$
there is no way to conclude $u=v$ from $a=b$.
One solution is to use the instance $I(a,b,u,v) \lor a=b$ of (I-intro2).
Then one branch is immediate and in the other branch
we can infer $P(b,u)$ from $P(a,u)$ by equality reasoning and use (I-intro1). 
\begin{align*}
&I(a,b,u,v),\bot\\
...
$T$ by leaving out the $f$-axioms and adding $\vdash_{{y}} \exists u P({y},u)$.
Then we have:
\begin{enumerate}
...
Now build a proof of $\Gamma,\Sigma\vdash_{\vec{x}}^{T_0} A$ from first 
