...
If $\Gamma\vdash_{\vec{x}}^T A$, $\vec{x}\subseteq \vec{{sec:skolemconstant}y}$, $\Gamma\subseteq \Sigma$ and all free variables of $\Sigma$ are in $\vec{y}$, then $\Sigma\vdash_{\vec{y}}^T A$.
\end{lemma}
...
contains the term $r$. For example, if  $\phi(x) = P(r,x{sec:skolemconstant})$
...
The combination of Lemma~\ref{subst1} and \ref{subst2} m{sec:skolemconstant}akes
...
Assume all free variables of $\vec{t}$ are in $\vec{x}$. If the derivation
of $\Gamma\vdash_{\vec{x}}^T A$ does not use the instance $P(\vec{t},f(\vec{t}))$
...
Here we use that $f$ has positive arity, and that the case of a 
...
Then use the induction hypothesis and the idempotency of sets.
\end{proof}

The combination of the previous two lemmas allows us to replace
under suitable conditions an application of $\vdash_{\vec{y}} P(\vec{y},f(\vec{y}))$
by one of $\vdash_{\vec{y}} \exists u P(\vec{y},u)$.

\begin{lemma}\label{lem:elimPtft}
Let $T$ consist of axioms not mentioning $f$ including $\vdash_{\vec{y}} \exists u P(\vec{y},u)$,  
...
Let $T'$ consist of axioms not
mentioning $f$ plus the axiom $\vdash_{\vec{y}} P(\vec{y},f(\vec{y}))$.
Let $T$ be $T'$ with the only axiom mentioning $f$ replaced by
...
\begin{proof}
Let $T_f$ be the theory $T$ with the signature extended with $f$. 
First apply Lemma~\ref{lem:upfront} to eliminate all instances of
$\vdash_{\vec{y}} P(\vec{y},f(\vec{y}))$ used in the derivation of
$\Gamma\vdash_{\vec{x}}^{T'} A$. This results in a derivation
$\Gamma,P(\vec{t}_1,f(\vec{t}_1)),\ldots,P(\vec{t}_n,f(\vec{t}_n))\vdash_{\vec{x}}^{T_f} A$.
Without loss of generality we may assume that the $P$-atoms are ordered such
that $i<j$ whenever $f(\vec{t}_i)$ occurs in $\vec{t}_j$. TBE
\end{proof}

> any term $f(\vec{t})$ contains a variable that we can use as $u$. Here
> $f$ must have positive arity (the case of a Skolem constant
> has been settled in Section~\ref{sec:skolemconstant}).
