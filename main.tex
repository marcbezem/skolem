...
%including $\vdash_{\vec{y}} \exists u P(\vec{y},u)$,  
plus the $f$-axioms $\vdash_{\vec{y}} P(\vec{y},f(\vec{y}))$
...
Let $T$ with equality consist of axioms not mentioning $f$, 
including the axiom $\vdash_{\vec{y}} \exists u P(\vec{y},u)$,  
plus the $f$-axioms $\vdash_{\vec{y}} P(\vec{y},f(\vec{y}))$
and $x=x'\vdash_{x,x'} f(x) = f(x')$.
Assume all free variables of $\vec{t}$ are in $\vec{x}$.
Consider a derivation of $\Gamma\vdash_{\vec{x}}^T A$
that is $f(t)$-safe. Then there is a derivation
of $\Gamma,\Sigma\vdash_{\vec{x}}^T A$ that is
not using instances of the $f$-axioms with a term $s$
such that 
%Let $\Sigma$ consist of atoms $P(s,f(s))$ and $f(s) = f(s')$
%with all free variables in $\vec{x}$.
%Then $\Gamma,\Sigma\vdash_{\vec{x}}^T A$
%by a derivation not using the instances of  $\vdash_{\vec{y}} P(\vec{y},f(\vec{y}))$
%or $x=x'\vdash_{x,x'} f(x) = f(x')$ concluding atoms in $\Sigma$.
\end{lemma}

\begin{proof}
Similar to the proof of Lemma~\ref{lem:upfront}.
\end{proof}
\medskip
