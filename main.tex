...
that is used in the derivation, we have $\Gamma \vdash_{\vec{x}}^{T} t=s'$ ($t=s$)
whenever $\Gamma \vdash_{\vec{x}}^{T} t=s$ ($t=s'$).
$\Gamma,A$  is called $f(t)$-\emph{safe} if $\Gamma,A$ does not mention 
any term $f(s)$ such that $\Gamma \vdash_{\vec{x}}^{T} t=s$.
\end{definition}
