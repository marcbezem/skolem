...
by a forcing relation on a suitable site.

%FALSE in 6.2? The examples in this section are such that the
%skolemized sentence is not true in the generic model,
%so that their conservativity does not follow from 
%elementary semantic observations.
...
$\Gamma,\,t = 0\vdash_{\vec{x}}^{T} A$ by taking $u=0$. On the other hand we can get
$\Gamma,\,t y = 1\vdash_{\vec{x},y}^{T} A$ by taking $u=1$. 
Using the field axiom we get  $\Gamma\vdash_{\vec{x}}^{T} A$.
...
$$
Skolemizing here is a little more complicated since we have proved 
Skolem's Theorem only for atomic formulas. That issue can easily
be fixed by taking suitable definitional extensions. 
With this in mind one sees that it is conservative to introduce 
two Skolem functions $J(x)$ and $K(x)$ with the rule
$$
...
have no Skolem function in the generic model, and thus, 
we cannot simply rely on a semantical argument.
