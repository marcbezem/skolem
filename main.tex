...
Assume $u$ and all free variables of $\vec{t}$ are in $\vec{x}$. If the derivation
of $\Gamma\vdash_{\vec{x}}^T A$ does not use the instance $P(\vec{t},f(\vec{t}))$
of the only axiom in $T$ mentioning $f$, then $\Gamma\rho\vdash_{\vec{x}}^T A\rho$.
\end{lemma}
...
we use that $f$ has positive arity, and that the case of a Skolem constant
