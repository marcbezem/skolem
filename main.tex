...
The examples in this section are such that the
...
Skolem's Theorem only for atomic formulas. This issue can easily
be fixed by some definitional extensions. With this in mind one sees that
it is conservative to introduce two Skolem functions $J(x)$ and $K(x)$ with the rule
$$
\vdash_x xJ(x) = 1~\vee~(1-x)K(x) = 1
$$
There are no such functions in the generic model.

 since these functions should be defined
