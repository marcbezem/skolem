...
Consider the axiom $\vdash_{x,y} M(x,y,x)  \lor M(x,y,y)$.
Assume we conclude $\Gamma\vdash_{\vec{x}}^{T_0} A$ from 
$\Gamma, M(x,y,x) \vdash_{\vec{x}}^{T_0} A$ and
$\Gamma, M(x,y,y) \vdash_{\vec{x}}^{T_0} A$. By weakening we then also have
$\Gamma, y\leq x, M(x,y,x)\vdash_{\vec{x}}^{T_0} A$ and
$\Gamma, x\leq y, M(x,y,y)\vdash_{\vec{x}}^{T_0} A$.
But then we can also conclude $\Gamma\vdash_{\vec{x}}^{T_0} A$
by using the axioms of $T_0$ (totality and M-intro).
...
%(The historic source, Skolem's paper \cite{Skolem}, was available from the cellar of CAS.) 
Both authors express their gratitude to
Gilles Dowek and Ulrik Buchholtz for many interesting discussions.
